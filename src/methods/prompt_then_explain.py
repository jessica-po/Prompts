...
class PromptThenExplainRunner:
    ...
    def _resolve_prompts(self, fallback_system: str, fallback_user: str, suffix: str = "") -> tuple[str, str]:
        method = str(self._context.get("method", "5_prompt_then_explain")).strip() or "5_prompt_then_explain"
        try:
    ...
    def run_sample(self, mapping: Dict[str, str], model: Any, system_template: str, user_template: str, gen_max_new_tokens: int | None = None) -> Dict[str, Any]:
        # Stage 1: prompt for solution/code directly
        ...
        if gen_max_new_tokens is not None:
            draft = model.generate(messages1, params=GenerationParams(max_new_tokens=gen_max_new_tokens))
        else:
            draft = model.generate(messages1)

        # Stage 2: explain reasoning or verification based on the draft
        s2_sys, s2_user = self._resolve_prompts(system_template, user_template, suffix="_2")
        mapping2 = dict(mapping)
        mapping2["draft"] = draft
        mapping2["previous_output"] = draft
        ...
        return {
            ...
            "stages": [
                {"stage": 1, "messages": messages1, "completion": draft},
...
register_method("5_prompt_then_explain", PromptThenExplainRunner)
